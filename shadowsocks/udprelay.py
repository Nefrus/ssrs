...
class UDPRelay(object):
    ...
    def __init__(self, config, dns_resolver, is_local, stat_callback=None, stat_counter=None):
        # 配置文件赋值
        self._config = config
        # 若 config.get('connect_verbose_info', 0) > 0
        # common.connect_log = logging.info
        if config.get('connect_verbose_info', 0) > 0:
            common.connect_log = logging.info
        # 如果是本地
        # 设置 _listen_addr 为 local_address
        # 设置 _listen_port 为 local_port
        # 设置 _remote_addr 为 server
        # 设置 _remote_port 为 server_port
        if is_local:
        ...
        # 若不是本地
        # 设置 _listen_addr 为 server
        # 设置 _listen_port 为 server_port
        # 设置 _remote_addr 为 None
        # 设置 _remote_port 为 None
        else:
        ...
        print( server_info.host,server_info.port ,server_info.users )
